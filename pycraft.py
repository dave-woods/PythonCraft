...
wI = {"tree" : pyneitems.NaturalItem("tree", 0.8, destroyedBy=["axe"], destroyedTo=["log", "log"])}
...
class Actor(object):
    ...
    def breakItem(self, item):
        fragments = item.getFragments()
        for element in fragments:
            self.room.addToRoom(element)
        self.room.removeFromRoom(item)
        return fragments
        
class Player(Actor):
    def __init__(self, room):
        Actor.__init__(self, room)
        self._inventory = {}
    ...
    def _isInInventory(self, item, num=1):
        return item in self._inventory.keys() and self._inventory[item] >= num
    
    def pickUp(self, item):
        if self.room.existsInRoom(item):
            if item.isStorable():
                self.room.removeFromRoom(item)
                self._addToInventory(item)
    ...
    def place(self, item):
        if self._isInInventory(item):
            self.room.addToRoom(item)
            self._removeFromInventory(item)
    ...
    def punch(self, item):
        if self.room.existsInRoom(item):
            print("You punched the " + str(item) + "...")
            if item.isDestroyedBy("punch"):
                fragments = self.breakItem(item)
                print("It broke into " + str(fragments) + "!")
                return True
            elif item == "boulder":
                self.room.removeFromRoom(item)
                self.room.addToRoom("rock")
    ...
    def swingAxe(self, item):
        if self.room.existsInRoom(item):
            print("You swung your axe at the " + str(item) + "...")
            if item.isDestroyedBy("axe"):
                fragments = self.breakItem(item)
                print("It broke into " + str(fragments) + "!")
                return True
            else:
    ...
    def swingPickaxe(self, item):
        if self.room.existsInRoom(item):
            print("You swung your pickaxe at the " + str(item) + "...")
            if item.isDestroyedBy("pickaxe"):
                fragments = self.breakItem(item)
                print("It broke into " + str(fragments) + "!")
                return True
            else:
...
class Room(object):
    ...
    def __contains__(self, key):
        return self.existsInRoom(key)
    """
    
    def addToRoom(self, item, num=1):
        if item in self._contents.keys():
            self._contents[item] += num
        else:
            self._contents[item] = num
            
    def removeFromRoom(self, item, num=1):
        if item in self._contents.keys():
            self._contents[item] -= num
    ...
    def populateRoom(self):
        for i in range(random.randrange(10, 20)):  # @UnusedVariable
            ...
            if naturalItems[item] > random.uniform(0.01, 1.00):
                self.addToRoom(item, random.randrange(1, 3))
                
    def existsInRoom(self, item, num=1):
        return item in self._contents.keys() and self._contents[item] >= num
        
    def lookAtRoom(self):
        for key in sorted(self._contents):
            print(key + " : " + str(self._contents[key]))
